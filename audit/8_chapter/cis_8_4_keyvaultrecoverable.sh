...
if [ "$(az resource show --id /subscriptions/$subscriptionID/resourceGroups/$ResourceGroup/providers/Microsoft.KeyVault/vaults/$KeyVault | sed -r 's/(\[|\])//g' | sed -r 's/(\{|\})//g' | sed '/^$/d' | sed 's/"//g' | sed 's/ //g')" ]; then
