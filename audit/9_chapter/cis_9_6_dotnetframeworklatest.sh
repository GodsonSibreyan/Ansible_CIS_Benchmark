...
do
if [ "$(az webapp config show --resource-group $ResourceGroup --name $WebApp --query netFrameworkVersion | grep "v4.0" | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
if [ "$count" -eq 0 ]; then
    ...
    echo "9.6 Ensure that '.Net Framework' version is the latest, if used as a part of the web app (Not Scored)  FAILED"
fi