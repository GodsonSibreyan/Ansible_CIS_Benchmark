...
do
if [ "$(az webapp config show --resource-group $ResourceGroup --name $WebApp --query pythonVersion | sed '/"//g' | sed '/^$/d' | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
if [ "$count" -eq 0 ]; then
    echo "9.8 Ensure that 'Python version' is the latest, if used to run the web app (Not Scored)  FAILED"
else
    echo "PASSED"
fi