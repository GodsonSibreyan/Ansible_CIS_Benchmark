...
do
if [ "$(az postgres server configuration show --resource-group $ResourceGroup --server-name $PostgresDB --name log_disconnections | grep value | grep off | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
...
if [ "$count" -eq 0 ]; then
    ...
    echo "4.15 Ensure server parameter 'log_disconnections' is set to 'ON' for PostgreSQL Database Server (Scored) FAILED"
fi