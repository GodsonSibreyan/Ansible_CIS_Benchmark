...
do
for SQLDB in `az sql db list --resource-group $ResourceGroup --server $DBServer | grep name | gawk -F' ' {'print $2'} | sed 's/master//g' | sed 's/Basic//g' | sed 's/Standard//g' | sed 's/Premium//g' | sed 's/General Purpose//g' | sed 's/Hyperscale//g' | sed 's/Business Critical//g' | sed 's/[^a-z  A-Z]//g' | sed 's/ //g'`;
do
if [ "$(az sql db tde show --resource-group $ResourceGroup --server $DBServer --database $SQLDB --query status | grep "Disabled" | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
