...
do
if [ "$(az postgres server configuration show --resource-group $ResourceGroup --server-name $PostgresDB --name log_duration | grep value | grep off | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
...
if [ "$count" -eq 0 ]; then
    ...
    echo "4.16 Ensure server parameter 'log_duration' is set to 'ON' for PostgreSQL Database Server (Scored)  FAILED"
fi