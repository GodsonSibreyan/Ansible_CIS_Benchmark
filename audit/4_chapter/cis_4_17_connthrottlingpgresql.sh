...
do
if [ "$(az postgres server configuration show --resource-group $ResourceGroup --server-name $PostgresDB --name connection_throttling | grep value | grep on | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
...
if [ "$count" -eq 0 ]; then
    echo "4.17 Ensure server parameter 'connection_throttling' is set to 'ON' for PostgreSQL Database Server (Scored)  FAILED"
else
    echo "PASSED"
fi