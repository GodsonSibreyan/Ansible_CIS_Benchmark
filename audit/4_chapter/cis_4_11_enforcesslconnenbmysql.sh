...
do
for MySQLDB in `az mysql server list -g godsons | grep name | sed 's/B_Gen5_2//g' | sed 's/GP_Gen5_4//g' | sed 's/MO_Gen5_4//g' | sed 's/name//g' | sed 's/"//g' | sed 's/,//g' | sed 's/://g' | sed 's/ //g'`;
do
if [ "$(az mysql server show --resource-group $ResourceGroup --name $MySQLDB --query sslEnforcement | grep "Disabled" | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
