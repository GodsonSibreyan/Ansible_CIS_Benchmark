...
do
if [ "$(az postgres server configuration show --resource-group $ResourceGroup --server-name $PostgresDB --name log_checkpoints | grep value | grep on | wc -l)" -gt "0" ]; then
        count=$((count+1))
    fi
done
done
...
if [ "$count" -eq 0 ]; then
    echo "4.12 Ensure server parameter 'log_checkpoints' is set to 'ON' for PostgreSQL Database Server (Scored)   FAILED"
else
    echo "PASSED"
fi
