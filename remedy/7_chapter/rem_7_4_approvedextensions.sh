...
do
for Extension in `az vm extension list --vm-name $VMname --resource-group $ResourceGroup --query [*].name | sed 's/,//g' | sed 's/"//g' | sed -r 's/(\[|\])//g' | sed '/^$/d' | sed 's/ //g'`;
do
# 7.4 Ensure that only approved extensions are installed (Not Scored) 

if [ "$(az vm extension delete --resource-group $ResourceGroup --vm-name $VMname --name $Extension | wc -l)" -eq 0 ]; then
