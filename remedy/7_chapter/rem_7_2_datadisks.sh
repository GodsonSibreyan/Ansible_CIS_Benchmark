...
if [ "$(az vm encryption enable --name $VMname --resource-group $ResourceGroup --volume-type DATA --aad-client-id $ClientIDAADApp --aad-client-secret $ClientSecretAADApp --disk-encryption-keyvault $KeyVaultHost)" ]; then
   echo "PASSED"
else
   echo "7.2 Ensure that 'Data disks' are encrypted (Scored) "
fi
done
done
done
